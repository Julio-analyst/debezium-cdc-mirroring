...
$LogPath = "testing-results/stress-test-$(Get-Date -Format 'yyyy-MM-dd-HH-mm').log"
$ResourceLogPath = "testing-results/resource-usage.log"
...
function Write-ResourceUsage {
    ...
    try {
        $dockerStats = docker stats --no-stream --format "table {{.Name}}\t{{.CPUPerc}}\t{{.MemUsage}}" 2>$null | Out-String
    } catch { $dockerStats = "(docker stats unavailable)" }
    Add-Content -Path $ResourceLogPath -Value "$logLine"
    if ($dockerStats) {
        Add-Content -Path $ResourceLogPath -Value ("DOCKER STATS:" + [Environment]::NewLine + $dockerStats.Trim())
    }
}
